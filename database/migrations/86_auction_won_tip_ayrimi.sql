-- ============================================
-- Migration 86: Açık Arttırma Tip Ayrımı
-- arabamabak - masaya oturma bloke vs kazanma modül ayrımı
-- ============================================
-- SORUN: auction_won tipi hem masaya oturma bloke hem
-- kazanma modül ücreti için kullanılıyordu. Dashboard
-- "Açık Arttırma Modül Ücreti" kartı karışık gösteriyordu.
--
-- ÇÖZÜM: Tip ayrımı yap.
--   - Masaya oturma:        auction_seat_hold   (bloke, geçici)
--   - Masadan ayrılma:     auction_seat_release (iade, audit)
--   - Kazanma modül:       auction_won          (KALICI)
--   - Kaybetme iade:       auction_seat_release (iade, audit)
--
-- DOKUNULMAYAN:
--   - profile.wallet_balance (zaten doğru, fonksiyonlar UPDATE ediyor)
--   - auction_seat_holds (seat durumu zaten doğru)
--   - transactions.amount (sadece tip değişiyor)
--   - deposit transaction'lar (dün/evvelsi gün)
-- ============================================

-- 1) fn_seat_hold_join: INSERT type auction_won -> auction_seat_hold
...
BEGIN
  IF NEW.status = 'holding' AND (OLD IS NULL OR OLD.status <> 'holding') THEN
    ...
    VALUES (NEW.user_id, NEW.amount, v_new_balance, 'completed', 'auction_seat_hold', 'Masa bloke', NEW.auction_id, 'wallet', 'seat-hold-' || NEW.id::text, now(), now());
...
-- 2) fn_seat_hold_leave: INSERT type auction_refund -> auction_seat_release
...
-- 3) fn_auction_ended_seat_settle: kaybeden LOOP tip auction_refund -> auction_seat_release
--    (Kazanan kısım auction_won KALIR - modül ücreti)
...
BEGIN
  IF NEW.status IN ('ended', 'sold')
    ...
    -- KAYBEDENLER: +500 bloke iade (net 0), tip: auction_seat_release
    FOR v_loser_seat IN
...
-- Tetikleyici güvende olmak için (değişiklik yok, sadece güvende)
