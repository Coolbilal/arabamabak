-- 1) admin_permissions
ALTER TABLE public.admin_permissions ADD COLUMN IF NOT EXISTS sub_area TEXT;
ALTER TABLE public.admin_permissions ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT now();

-- 2) admin_users
ALTER TABLE public.admin_users ADD COLUMN IF NOT EXISTS custom_role TEXT;
ALTER TABLE public.admin_users ADD COLUMN IF NOT EXISTS last_sign_in_at TIMESTAMPTZ;
ALTER TABLE public.admin_users ADD COLUMN IF NOT EXISTS must_change_password BOOLEAN DEFAULT true;
ALTER TABLE public.admin_users ADD COLUMN IF NOT EXISTS invitation_accepted_at TIMESTAMPTZ;

-- 3) Index
CREATE UNIQUE INDEX IF NOT EXISTS idx_admin_permissions_unique ON admin_permissions (user_id, area, COALESCE(sub_area, ''));
...
CREATE POLICY activity_select ON public.admin_activity_logs FOR SELECT USING (
...
CREATE POLICY activity_insert ON public.admin_activity_logs FOR INSERT WITH CHECK (true);

-- 5) Trigger fonksiyonu
CREATE OR REPLACE FUNCTION public.log_admin_action()
RETURNS TRIGGER AS $$
DECLARE
  v_admin_id UUID;
BEGIN
  SELECT id INTO v_admin_id FROM public.admin_users WHERE user_id = auth.uid() AND is_active = true LIMIT 1;
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    VALUES (v_admin_id, 'insert', TG_TABLE_NAME, NEW.id, jsonb_build_object('new', to_jsonb(NEW)));
    RETURN NEW;
  ELSIF TG_OP = 'UPDATE' THEN
    INSERT INTO public.admin_activity_logs (admin_user_id, action, resource_type, resource_id, details)
    VALUES (v_admin_id, 'update', TG_TABLE_NAME, NEW.id, jsonb_build_object('old', to_jsonb(OLD), 'new', to_jsonb(NEW)));
    RETURN NEW;
  ELSIF TG_OP = 'DELETE' THEN
    INSERT INTO public.admin_activity_logs (admin_user_id, action, resource_type, resource_id, details)
    VALUES (v_admin_id, 'delete', TG_TABLE_NAME, OLD.id, jsonb_build_object('old', to_jsonb(OLD)));
    RETURN OLD;
  END IF;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 6) Trigger'lar (sadece en önemli 2-3 tablo)
